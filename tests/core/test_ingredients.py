...
class TestPopulationIngredients:
    ...
    def test_initialization(self, pop_ingredients):
        ...
        assert hasattr(pop_ingredients, 'imf_function'), (
            'Should have IMF function'
        )
    ...
    def test_has_isochrone_data(self, pop_ingredients):
        ...
        assert hasattr(pop_ingredients, 'mass_ini_data'), (
            'Should have mass data'
        )

    def test_has_solar_reference(self, pop_ingredients):
        """Test that solar reference is loaded."""
        assert hasattr(pop_ingredients, 'sun_spec'), (
            'Should have solar spectrum'
        )
        assert hasattr(pop_ingredients, 'sun_vmag'), (
            'Should have solar V magnitude'
        )
    ...
    def test_get_isochrone_returns_four_values(self, pop_ingredients):
        ...
        assert len(result) == 4, (
            'Should return 4 arrays (mass, teff, logg, lumi)'
        )
