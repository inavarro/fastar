...
class TestIsochroneInterpolation:
    ...
    def test_isochrone_interpolation_finite_values(self):
        ...
        mass_ini_data = jnp.linspace(0.1, 10, n_tracks)[
            :, None, None
        ] * jnp.ones((n_tracks, len(ages), len(mets)))
        teff_out_data = jnp.ones((n_tracks, len(ages), len(mets))) * 5000
        logg_out_data = jnp.ones((n_tracks, len(ages), len(mets))) * 4.0
        lumi_out_data = jnp.ones((n_tracks, len(ages), len(mets)))
    ...
    def test_isochrone_interpolation_output_shape(self):
        ...
        assert mass_ini.shape == (n_tracks,), (
            f'Expected shape ({n_tracks},), got {mass_ini.shape}'
        )
        assert teff_out.shape == (n_tracks,), (
            f'Expected shape ({n_tracks},), got {teff_out.shape}'
        )
        assert logg_out.shape == (n_tracks,), (
            f'Expected shape ({n_tracks},), got {logg_out.shape}'
        )
        assert lumi_out.shape == (n_tracks,), (
            f'Expected shape ({n_tracks},), got {lumi_out.shape}'
        )
