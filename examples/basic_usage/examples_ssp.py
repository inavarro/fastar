...
ml_g = ssp_spec.mass_to_light_ratio(age=age, met=met, imf_params={"alpha": imf_slope}, filter_response=gtrans)
...
wave, error = ssp_spec.synthesize_nsim(age=age, met=met, imf_params={"alpha": imf_slope})
...
print('It takes '+str(np.round(time.time()-t0,2))+'s to calculate 50 SSPs' )
...
itrans = np.interp(phot_wave, data['Wavelength'], data['Transmission'],left=0,right=0)
imag = compute_ab_magnitudes(phot_wave, phot_spec, itrans)

ml_i = ssp_phot.mass_to_light_ratio(age=age, met=met, imf_params={"alpha": imf_slope}, filter_response=itrans)