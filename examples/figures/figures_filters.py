...
plt.plot(
    mwave,
    mspec / np.mean(mspec[(mwave > 5000) & (mwave < 5100)]),
    label='E-MILES (12 Gyr, Z=+0.06)',
    color='grey',
    lw=1.4,
    alpha=0.8,
)
plt.plot(
    phot_wave,
    phot_spec / np.mean(phot_spec[(phot_wave > 5000) & (phot_wave < 5100)]),
...
plt.plot(
    wave,
    spec / np.mean(spec[(wave > 5000) & (wave < 5100)]),
    label='Spectroscopic SSP',
    color='xkcd:coral',
    lw=1.2,
)
